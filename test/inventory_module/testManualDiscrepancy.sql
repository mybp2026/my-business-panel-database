...
DO $$
DECLARE
    v_tenant_id uuid := (SELECT tenant_id FROM core.tenant LIMIT 1);
...
BEGIN
    ...
    SELECT product_id INTO v_product_id FROM core.product WHERE tenant_id = v_tenant_id LIMIT 1;
    IF v_product_id IS NULL THEN
        DELETE FROM inventory_module.warehouse WHERE warehouse_id = v_wh_id;
        RAISE EXCEPTION 'No hay product en core.product. Inserte al menos un registro en core.product';
    END IF;
