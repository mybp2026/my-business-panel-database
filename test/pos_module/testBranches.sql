...
begin
    -- Obtener IDs
    select tenant_id into v_tenant_id from core.tenant where tenant_name = 'Comercio MultiSucursal';
    select branch_id into v_branch_centro from core.branch where tenant_id = v_tenant_id and branch_name = 'Sucursal Centro';
    select branch_id into v_branch_norte from core.branch where tenant_id = v_tenant_id and branch_name = 'Sucursal Norte';
    select tenant_customer_id into v_cliente_id from core.tenant_customer where tenant_id = v_tenant_id and email = 'ana.ramirez@email.com';
    select product_id into v_prod_a from core.product where tenant_id = v_tenant_id and sku = 'PROD-A';
    select product_id into v_prod_b from core.product where tenant_id = v_tenant_id and sku = 'PROD-B';
    select product_id into v_prod_c from core.product where tenant_id = v_tenant_id and sku = 'PROD-C';
...
join core.branch b on s.branch_id = b.branch_id
join core.product p on si.tenant_id = p.tenant_id and si.product_id = p.product_id
