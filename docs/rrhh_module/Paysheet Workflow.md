...
This document explains the paysheet (payroll) flow in the RRHH module: how a paysheet and its details are created, how income entries update gross salary via triggers, how recalculation states are enforced, how the paysheet is closed, and how to generate a monthly CCSS report. The flow references `rrhh_module` tables, triggers, and functions defined in the project.
...
- Core data: a `core.branch` to attribute the paysheet, and valid `core.payment_method` IDs for employee payments in `paysheet_detail`.
- Employees: existing employees in `rrhh_module.employee` (can be created via `rrhh_module.create_new_employee`).
- RRHH objects: deployed `rrhh_module.paysheet`, `rrhh_module.paysheet_detail`, `rrhh_module.income_concept`, `rrhh_module.income_register`, and status catalog `rrhh_module.paysheet_status` with at least `Pending`, `Completed`, `Canceled`.
- Triggers & functions: trigger `update_gross_salary` over `income_register`; trigger `protect_net_salary` over `paysheet_detail`; functions `rrhh_module.update_paysheet_state` and `rrhh_module.generate_monthly_ccss`.
...
5. Close the paysheet by calling `rrhh_module.update_paysheet_state(p_paysheet_id)`. It verifies no detail requires recalculation and sets status to `Completed`.
6. Generate the monthly CCSS report via `rrhh_module.generate_monthly_ccss(p_year, p_month)
...
```sql
INSERT INTO rrhh_module.income_concept (concept_name, calculation_type, ccss_apply, tax_apply)
...
SELECT rrhh_module.update_paysheet_state(<paysheet_id>);

-- Verify closure
SELECT p.paysheet_id, ps.status_description
FROM rrhh_module.paysheet p
JOIN rrhh_module.paysheet_status ps ON ps.status_id = p.paysheet_status_id
...
- Function `rrhh_module.update_paysheet_state(p_paysheet_id)`:
  - Fails if any `paysheet_detail.recalc_needed = TRUE`.
  - Sets `paysheet.paysheet_status_id` to `Completed` otherwise.
- Function `rrhh_module.generate_monthly_ccss(year, month)` aggregates CCSS totals from completed paysheets.
...
FROM rrhh_module.income_register ir
JOIN rrhh_module.income_concept ic ON ic.income_id = ir.concept_id
