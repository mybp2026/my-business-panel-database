-- ======================================================
-- MIGRATION: pos/037-fix-arqueo-per-method.sql
-- Audit:
--   Migration 036 compared p_closing_amount (total) against
--   expected_total (opening + all system sales). This causes
--   mismatch = false whenever the declared grand total matches,
--   even if individual methods don't.
--
--   Correct logic per business rules:
--     * cash:     user_cash     == opening_amount + system_cash  → square
--     * debit:    user_debit    == system_debit                  → square
--     * credit:   user_credit   == system_credit                 → square
--     * transfer: user_transfer == system_transfer               → square
--   Any method off by > 0.01 triggers mismatch.
--   mismatch_amount = net difference across all methods.
--   mismatch_type   = surplus if net > 0, shortage if net < 0.
--   p_closing_amount is stored as closing_amount for reference only;
--   it no longer drives the mismatch check.
-- ======================================================
...
BEGIN
    ...
    -- Aggregate system totals into session_payment_method_sales
    ...
    -- Per-method diff:
    --   cash expected = opening_amount + system_cash (user holds opening float + cash sales)
    --   debit/credit/transfer expected = system amount exactly
...
-- ROLLBACK
-- ======================================================
-- Restore 036 mismatch logic (total-based comparison):
--
-- CREATE OR REPLACE FUNCTION pos_schema.close_cash_register_session(
--     p_session_id     uuid,
--     p_closing_amount numeric,
